...
boolean Adafruit_FONA::UDPconnect(char *server, uint16_t port) {
	...
	if (!sendCheckReply(F("AT+CGATT?"), F("+CGATT: 1"), 20000)) return false;

	// Display IP/Port of sender
	if (!sendCheckReply(F("AT+CIPSRIP=1"), F("OK"), 20000)) return false;

	// Add IP head in recv data
	if (!sendCheckReply(F("AT+CIPHEAD=1"), F("OK"), 20000)) return false;

	// Enable the UDP Extended Mode
	if (!sendCheckReply(F("AT+CIPUDPMODE=1"), ok_reply)) return false;
...
boolean Adafruit_FONA::UDPsend(String packet) {
